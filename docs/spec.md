...
> ⚠ 아래 화면에 뜨는 수치는 D 초안이며 A 확정 전까지 기획서 본문에 인용하지 않는다(§7 상단 경고와 동일).
...
- 산식: `raw = 손실 상태 추가매수 건수 / 전체 매수 건수` → `score = raw × 100`
  - 진입 매수는 **분자에서만** 빠지고 분모(전체 매수 건수)에는 남는다 — §5.3 과 분모 정의가 다르다(§10 한계).
...
- 산식: `raw = 급등 후 매수 건수 / 전체 매수 건수` → `score = raw × 100`
  - 분모는 **전체 매수 건수**다(리더 결정 — 복원 예정). 보유 이력이 없어 전일 종가를 몰라 판정할 수 없는 매수는 분모에 남되 **분자에 0으로 들어간다**. 판정 가능 건수만 분모로 쓰면 판정 가능 건이 seed당 0~8건까지 떨어져 점수가 0~100을 오간다(§5.4).
...
- TODO: 합리형 페르소나 chasing 기준선 50.00 — **부분 처리**.
  ...
  **분산은 축소**(`api/service.py` `_reference_scores`, `_REFERENCE_UNIVERSE`).
  다만 합리형이 대조군으로 분리되는지는 표본을 넓히는 걸로 해결되지 않고,
  **§5.3 분모를 전체 매수 건수로 복원**해야 풀린다(리더 결정, 복원 예정).
  표본이 여전히 작아(seed당 표준편차 상당) 백분위는 참고용 이상은 아님.
...
- 지표별 분모 정의가 다르고(물타기는 진입 매수를 분자에서만 제외, 추격매수는 전체 매수 분모 — §5.2·§5.3), 종합 점수 가중치 40/35/25 는 룰 `MAX_CONTRIBUTION` 을 그대로 재사용한 것이라 근거가 약하다
