...
⚠ 이 파일은 test-jw 브랜치의 초안이다. A(리더) 가 오늘 밤 확인하기 전, docs/schema.md
§6 미결 3개는 같은 문서에 B(이 초안 작성자)가 남긴 제안값으로 임시 확정하고 짰다
(§6.1 amount 기준 평단가 / §6.2 T+2 보정 없음 / §6.3 동일일 다중체결 그대로 둠).
A 가 다르게 결정하면 아래 세 지점만 고치면 된다 — 각 지점에 "§6.N 제안" 으로 표시.

── 알고리즘 ──────────────────────────────────────────────────────────────
종목별로 독립적으로 하루씩 걸어간다(포지션은 종목마다 완전히 별개라서). 하루치
처리 순서:
  1. 그날 체결된 거래(§6.3 제안: 동일일 여러 건이면 source_row 순서대로 전부 적용)
     - BUY: 진입(직전 수량 0) 이면 새 episode 시작, 아니면 추가매수(add_buy_count+=1).
       평단가는 §6.1 제안대로 amount(정산금액) 기준: avg_cost = 누적(BUY.amount) / 누적(수량)
