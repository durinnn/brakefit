...
시나리오 (as_of = 2026-08-04 화, 손계산 가능한 크기):
    삼성전자(005930): **보유 중**(에피소드 열림). 20주 · 평단 67,000원
        as_of 직전 영업일(08-03) 종가 71,000원
        → 평가이익 = (71,000 − 67,000) × 20 = +80,000원 → 발동
    카카오(035720): **보유 중**. 30주 · 평단 39,000원, 08-03 종가 37,500원
...
PRICES = FakePriceSource(
    {
        "005930": {"2026-08-03": 71_000, "2026-08-04": 40_000},  # 당일값은 쓰면 안 됨
        "035720": {"2026-08-03": 37_500, "2026-08-04": 99_000},
...
def test_평가이익_종목_매도는_발동한다():
    ...
    assert result.evidence[0]["date"] == "2026-08-03"  # as_of 당일이 아니라 직전 영업일
...
def test_평가손익은_timeline_이_아니라_직전_종가로_계산한다():
    """timeline.close 가 오래된 값이어도 판정은 as_of 직전 종가를 따른다.

    timeline 행은 이익(+80,000)인데 08-03 종가가 평단 아래면 손실로 뒤집혀야 한다.
    """
    dropped = FakePriceSource({"005930": {"2026-08-03": 60_000}})  # 평단 67,000 아래
    result = evaluate(
